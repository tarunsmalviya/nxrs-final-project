...
struct BoundingBox {
  ...
  double getYLen() { return maxy - miny; }
  double getXLen() { return maxx - minx; }
  double getZLen() { return maxz - minz; }
};
...
public:
  ...
  // Mesh Operations
  void
  removePoint(Point *p);    // Remove point p and all its faces from the surface
  void removeFace(Face *f); // Remove face f from the surface
  void removeEdge(Edge *e);
  void mergeVertices(Point *p1,
                     Point *p2); // Merge p1 and p2. The resulting vertex is
                                 // placed at p2's position. p1 is removed.
  bool collapse(Edge &);
  void simplify(float goal, const int);

  // ELen (Edge length) simplification operations
  void
  ELEN_computeEdgeCosts(); // Compute all edges costs and construct edge_queue
  void ELEN_updateEdgeCost(Point *v);
  double ELEN_computeCost(Edge *e);
  void ELEN_setPlacement(Edge *e); // Determine the placement vertex of an edge
                                   // e
