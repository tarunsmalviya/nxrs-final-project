...
Surface::Surface(string inputFile) {
  ...
  t = diff(t0, t1);
  cout << greentty << "Time_read_input_and_init_data: "
       << t.tv_sec * 1000 + t.tv_nsec / 1000000 << deftty << endl;
}
...
void Surface::readSurface(string inputFile) {
  ...
  // Set vertex pointers
  for (vector<Point *>::iterator it = m_points.begin(); it != m_points.end();
       ++it) {
    (*it)->pointer = it;
  }
  ...
  for (vector<Face *>::iterator it = m_faces.begin(); it != m_faces.end();
       ++it) {
    (*it)->pointer = it;
  }
...
void Surface::dumpBoundingBox() {
  ...
  fout << bbox.minx + bbox.getXLen() << " " << bbox.miny << " " << bbox.minz
       << endl;
  fout << bbox.minx + bbox.getXLen() << " " << bbox.miny + bbox.getYLen() << " "
       << bbox.minz << endl;
  fout << bbox.minx << " " << bbox.miny + bbox.getYLen() << " " << bbox.minz
       << endl;

  fout << bbox.minx << " " << bbox.miny << " " << bbox.minz + bbox.getZLen()
       << endl;
  fout << bbox.minx + bbox.getXLen() << " " << bbox.miny << " "
       << bbox.minz + bbox.getZLen() << endl;
  fout << bbox.minx + bbox.getXLen() << " " << bbox.miny + bbox.getYLen() << " "
       << bbox.minz + bbox.getZLen() << endl;
  fout << bbox.minx << " " << bbox.miny + bbox.getYLen() << " "
       << bbox.minz + bbox.getZLen() << endl;
...
void Surface::saveOFF(string output) {
  ...
  // cerr << "Output: " << valid_points.size() << " vertices - " <<
  // valid_faces.size() << " faces. \n"; fout << valid_points.size() << " " <<
  // valid_faces.size() << " 0\n";
  point_vec_it pend =
      remove_if(m_points.begin(), m_points.end(), [](const Point *p) {
        return (p->removed || p->faces.empty());
      }); // lambda
  m_points.erase(pend, m_points.end());

  face_vec_it fend =
      remove_if(m_faces.begin(), m_faces.end(),
                [](const Face *f) { return f->removed; }); // lambda function
  m_faces.erase(fend, m_faces.end());
  cerr << "Output: " << m_points.size() << " vertices - " << m_faces.size()
       << " faces.\n";
  ...
  for (std::vector<Face *>::iterator it = m_faces.begin(); it != m_faces.end();
    ...
    fout << "3"
         << " " << (*it)->points.at(0)->id << " " << (*it)->points.at(1)->id
         << " " << (*it)->points.at(2)->id << endl;
  }
...
void Surface::printFaces() {
  ...
  for (std::vector<Face *>::iterator it = m_faces.begin(); it != m_faces.end();
       ++it) {
    cout << (*it)->points.at(0) << " " << (*it)->points.at(1) << " "
         << (*it)->points.at(2) << endl;
  }
}
...
void Surface::removeFace(Face *f) {
  ...
  for (vector<Point *>::iterator it = f->points.begin(); it != f->points.end();
       ++it) {

    // cerr<< "p " << (*it)->id << endl;
    for (vector<Face *>::iterator it2 = (*it)->faces.begin();
         it2 != (*it)->faces.end(); ++it2) {
...
bool Surface::collapse(Edge &e) {
  ...
  if (p1->faces.size() == 0 ||
      p2->faces.size() == 0) // Do not simplify boundary corners
  {
  ...
  // clock_gettime(CLOCK_REALTIME,&t0);
  for (vector<Face *>::iterator fit = p1->faces.begin(); fit != p1->faces.end();
       ++fit) {
    bool removeface = false;
    vector<Point *>::iterator auxp1;
    for (vector<Point *>::iterator pit = (*fit)->points.begin();
         pit != (*fit)->points.end(); ++pit) {
      if ((*pit) == p2) {
        removeface = true;
        break;
      } else if ((*pit) == p1) {
  ...
  // cerr << "Removing faces: ";
  for (vector<Face *>::iterator it = for_removal.begin();
       it != for_removal.end(); ++it) {
    // cerr << (*it)->id << " ";
    removeFace(*it);
    (*it)->removed = true;
    // delete (*it);
  ...
  while (eit != p1->to.end()) {
    ...
    while (it != p2->to.end()) {
      Edge *e2 = (*it);
      if (e->p1->id == e2->p1->id) {
        // cerr << "Found " << e->id << " " << e->p1->id << " " << e->p2->id <<
        // endl;
  ...
  while (eit != p1->from.end()) {
    ...
    while (it != p2->from.end()) {
      Edge *e2 = (*it);
      if (e->p2->id == e2->p2->id) {
        // cerr << "Found from " << e->id << " " << e->p1->id << " " <<e->p2->id
        // << endl;
        edges_to_remove.push_back(e);
        found = true;
        break;
...
void Surface::initQuadrics() {
  ...
  // Quadric Q (4x4 matrix) is the sum of all planes tangent to a vertex v
  // (Garland, 97).
  for (point_vec_it pit = m_points.begin(); pit != m_points.end(); ++pit) {

    double Kp[4][4];
    for (face_vec_it fit = (*pit)->faces.begin(); fit != (*pit)->faces.end();
         ++fit) {
      ...
      // double d = vv->x*(*fit)->points[0]->x + vv->y*(*fit)->points[0]->y +
      // vv->z*(*fit)->points[0]->z; double plane_eq[4] = {vv->x, vv->y, vv->z,
      // d};
      //
      // //For this plane, the fundamental quadric Kp is the dot product of
      // plane_eq and plane_eq(transposed) (garland97) for(int i = 0; i < 4;
      // ++i)
      // {
      //     for(int j = 0 ; j < 4; ++j)
      //     {
